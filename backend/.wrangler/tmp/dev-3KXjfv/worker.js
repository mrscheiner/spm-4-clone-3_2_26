...
__name(getSeasonForLeague, "getSeasonForLeague");
var sportsdataRouter = createTRPCRouter({
  // returns league-wide games filtered to the given team
  // keep this a query so we can call it via GET; our client constructs the
  // URL manually and avoids any tRPC quirks.
  getSchedule: publicProcedure.input(
    objectType({
      leagueId: stringType(),
      teamId: stringType()
      // abbreviation, e.g. mil, bos
    }).optional()
    ...
    if (typeof globalThis !== "undefined") {
      console.log("[SD_PROXY] globalThis.request exists?", !!globalThis.request);
      if (globalThis.request) {
        console.log("[SD_PROXY] globalThis.request.url", globalThis.request.url);
      }
    }
    let key;
    ...
    console.log("[SD_PROXY] key lookup", { fromCtx: !!(ctx?.env && ctx.env.SPORTSDATA_API_KEY), fromProcess: !!(typeof process !== "undefined" && process.env.SPORTSDATA_API_KEY), fromGlobal: !!(typeof globalThis !== "undefined" && globalThis.SPORTSDATA_API_KEY) });
    if (!key) {
    ...
    const needsPatch = !patchedInput || typeof patchedInput !== "object" || Object.keys(patchedInput).length === 0 || !patchedInput.leagueId || !patchedInput.teamId;
    if (needsPatch) {
      try {
        if (ctx && ctx.req && ctx.req.url) {
          console.log("[SD_PROXY] parsing input from ctx.req.url", ctx.req.url);
          const url = new URL(ctx.req.url);
          const inp = url.searchParams.get("input");
          if (inp) {
            patchedInput = JSON.parse(inp);
          }
        } else if (typeof globalThis !== "undefined" && globalThis.request) {
    ...
    if (!patchedInput || typeof patchedInput !== "object" || !patchedInput.leagueId || !patchedInput.teamId) {
      ...
      const globalInfo = { globalThis: typeof globalThis !== "undefined" };
      if (typeof globalThis !== "undefined" && globalThis.request) {
        globalInfo.requestUrl = globalThis.request.url;
      }
      const ctxInfo = {};
      if (ctx && ctx.req) {
        ctxInfo.url = ctx.req.url || null;
        ctxInfo.headers = ctx.req.headers ? [...ctx.req.headers] : null;
      }
      return { events: [], error: "INVALID_INPUT", debug: { patchedInput, globalInfo, ctxInfo } };
    }
    ...
    let data = [];
    if (leagueKey === "nba") {
      const seasonYear = getSeasonForLeague(leagueKey);
      const seasonTypes = [`${seasonYear}PRE`, `${seasonYear}`, `${seasonYear}POST`];
      for (const sType of seasonTypes) {
        const url = `https://api.sportsdata.io/v3/nba/scores/json/SchedulesBasic/${sType}?key=${key}`;
        console.log(`[SD_PROXY] Fetching NBA SchedulesBasic`, url);
        try {
          ...
          if (res.ok) {
            ...
            if (Array.isArray(chunk) && chunk.length > 0) {
              ...
              const preseasonCount = chunk.filter((g) => g.SeasonType === 2).length;
              const regularCount = chunk.filter((g) => g.SeasonType === 1).length;
              const playoffCount = chunk.filter((g) => g.SeasonType === 3).length;
              console.log(`[SD_PROXY] Added ${chunk.length} games for ${sType} (Preseason: ${preseasonCount}, Regular: ${regularCount}, Playoff: ${playoffCount})`);
            }
          } else {
            console.error(`[SD_PROXY] HTTP ${res.status} for NBA SchedulesBasic ${sType}`);
          }
        } catch (e) {
          console.error(`[SD_PROXY] fetch failed for NBA SchedulesBasic ${sType}`, e);
        }
      }
      if (data.length > 0 && patchedInput && patchedInput.teamId) {
        const teamGames = data.filter((g) => g.HomeTeam === patchedInput.teamId.toUpperCase());
        const preseasonGames = teamGames.filter((g) => g.SeasonType === 2);
        console.log(`[SD_PROXY] Team ${patchedInput.teamId.toUpperCase()} has ${teamGames.length} home games, ${preseasonGames.length} preseason home games`);
      }
    } else {
      ...
      try {
        ...
        if (res.ok) {
          ...
          console.error("[SD_PROXY] HTTP", res.status, "for base schedule");
    ...
    try {
      ...
      const games = data.filter((g) => g.HomeTeam === abbrev).map((g, idx) => {
...
// .wrangler/tmp/bundle-j9KVKM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-j9KVKM/middleware-loader.entry.ts
