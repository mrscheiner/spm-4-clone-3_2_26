...
// .wrangler/tmp/bundle-XWDulT/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-XWDulT/middleware-insertion-facade.js
init_modules_watch_stub();
...
var MLS_TEAMS = [
  ...
  { id: "mtl-cf", name: "CF Montr\xE9al", leagueId: "mls", logoUrl: "https://a.espncdn.com/i/teamlogos/soccer/500/9720.png", primaryColor: "#0033A1", secondaryColor: "#000000", city: "Montreal", abbreviation: "MTL" },
...
var sportsdataRouter = createTRPCRouter({
  ...
  getLeagueScheduleAllTeams: publicProcedure.input(anyType()).query(async ({ input, ctx }) => {
    ...
    const requestedAbbr = parsedInput.teamAbbreviation ? String(parsedInput.teamAbbreviation).toUpperCase() : void 0;
    let key;
    if (ctx?.env && ctx.env.SPORTSDATAIO_API_KEY) {
      key = ctx.env.SPORTSDATAIO_API_KEY;
    ...
    const leagueKey = parsedInput.leagueId.toLowerCase();
    if (leagueKey === "mls") {
...
async function fetchMlsTeamSchedule(teamAbbreviation, season) {
  ...
  try {
    ...
    const homeGames = items.filter((match) => {
    ...
    const seenGames = /* @__PURE__ */ new Set();
    const deduplicatedHomeGames = homeGames.filter((match) => {
      const fields = match.fields || {};
      const slug = match.slug || "";
      const matchDate = (fields.matchDateTime || "").split("T")[0];
    ...
    const games = deduplicatedHomeGames.map((match) => {
      const fields = match.fields || {};
      ...
      const localDate = new Date(utcDate.getTime() + offsetHours * 60 * 60 * 1e3);
      return {
        ...
        opponentLogo: "",
        // Forge API doesn't include logos directly
        venueName: "",
        // Would need additional lookup
    ...
    if (teamOverrides && teamOverrides.length > 0) {
      ...
      for (const override of teamOverrides) {
        ...
        const gameExists = games.some(
          (g) => g.dateTimeISO.startsWith(datePrefix) || g.dateTimeISO.startsWith(nextDayPrefix) || g.id === override.id
        );
        if (!gameExists) {
          games.push({
            ...override,
...
__name(fetchMlsTeamSchedule, "fetchMlsTeamSchedule");
var mlsRouter = createTRPCRouter({
  /**
   ...
   */
  getTeamSchedule: publicProcedure.input(anyType()).query(async ({ input, ctx }) => {
    let parsedInput = void 0;
    ...
    const season = parsedInput.season || (/* @__PURE__ */ new Date()).getFullYear();
...
var worker_default = {
  async fetch(request, env, ctx) {
    ...
    return fetchRequestHandler({
      ...
      req: request,
      router: appRouter,
      createContext: /* @__PURE__ */ __name(({ req }) => ({ env, req }), "createContext")
    });
  }
};
