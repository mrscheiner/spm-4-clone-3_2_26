...
// .wrangler/tmp/bundle-gLu77w/checked-fetch.js
...
async function getTeamHomeSchedule({ league, teamId, season }) {
  ...
  const url = `https://api.sportsdata.io/v7/${league}/scores/json/Schedules/${seasonCode}?key=${key}`;
...
// .wrangler/tmp/bundle-gLu77w/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-gLu77w/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
app.get("/api/schedule", async (c) => {
  try {
    ...
    if (!league || !teamId || !season || type !== "home") {
      return c.json({ error: "Missing or invalid query params" }, 400);
    }
    const validLeagues = ["nhl", "nfl", "nba", "mls", "mlb"];
    if (!validLeagues.includes(league)) {
      return c.json({ error: "Invalid league" }, 400);
